...
Direct tools register from the metadata cache in the Pi agent dir (`~/.pi/agent/mcp-cache.json` by default, or `$PI_CODING_AGENT_DIR/mcp-cache.json` when set), so no server connections are needed at startup. On the first session after adding `directTools`, or after a server config change that invalidates the cache hash, those tools are not registered yet. Startup warns with the server name and configured tool names when known (or that names are unknown until discovery), warms metadata in the background, and tells you to restart after a successful warm. Discovery or OAuth failures are reported as errors instead of a silent empty tool set. `MCP_DIRECT_TOOLS=__none__` still disables direct tools with no extra warning. To force a refresh: `/mcp reconnect <server>`.
